    ...
    <div v-if="open" class="dtp-popover" ref="popover" tabindex="0" @keydown.enter.prevent="open=false">
      <div class="dtp-left">
        <div class="dtp-header">
          <div class="dtp-title" @click="panel = 'month'">{{ monthLabelEn }} {{ y }}</div>
          <div class="dtp-nav">
        ...
        <div class="dtp-calendar">
          <div v-if="panel === 'month'" class="ym-vertical" ref="yearList">
            <div v-for="yy in yearRange" :key="yy" class="dtp-year-group">
              <div :class="[`year-${yy}`, 'dtp-year-head', { sel: yy === y }]" @click="expandYear(yy)">{{ yy }}</div>
...
export default {
  ...
  methods: {
    toggle() { this.open = !this.open; if (this.open) { this.$nextTick(() => { const p = this.$refs.popover; if (p && p.focus) p.focus() }) } },
    chooseYear(yy) {
      this.y = yy
      this._emit()
    },
...
.ym-vertical { border: 1px solid #f3f4f6; border-radius: 6px; padding: 4px; }
...
.dtp-title { font-weight: 600; cursor: pointer; }
