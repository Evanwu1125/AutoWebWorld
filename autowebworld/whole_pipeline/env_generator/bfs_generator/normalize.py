...
def normalize_fsm(fsm: Dict[str, Any]) -> Dict[str, Any]:
    ...
    assert_true(isinstance(terminals, list) and len(terminals) > 0, "meta.terminal_pages must be a non-empty list")
    assert_true(isinstance(pages, list) and len(pages) > 0, "pages must be a non-empty list")
    ...
    assert_true(len(set(page_ids)) == len(page_ids), "page.id values must be unique")
