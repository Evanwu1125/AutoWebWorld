<template>
  <div class="min-h-screen bg-slate-50 flex items-center justify-center font-inter text-slate-900 px-4">
    <div class="bg-white p-8 rounded-lg shadow-lg border border-slate-200 max-w-md w-full text-center">
       ...
       <h2 class="text-2xl font-bold text-slate-900 mb-2">Assigned!</h2>
       <p class="text-slate-600 mb-8">{{ signatureStore.success_message || 'Ticket assigned successfully.' }}</p>
       
       <div class="space-y-3">
          <button id="assign-success-back-ticket" 
          ...
          <button id="assign-success-go-home" 
                  @click="handleGoHome" 
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
        signatureStore.success_message = "Ticket assigned successfully"
    })
