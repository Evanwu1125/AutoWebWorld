...
export default {
  name: 'DateTimePickerDateOnly',
  props: {
    id: { type: String, default: 'date-picker' },
    modelValue: { type: String, default: '2025-10-15' }, // 'YYYY-MM-DD' (date only)
  },
  ...
  computed: {
    ...
    yearRange() {
      ...
      const arr = []
      for (let yy = base - 6; yy <= base + 6; yy++) arr.push(yy)
      return arr
    },
    daysInMonth() {
  ...
  methods: {
    ...
    _emit() {
      const pad = (n) => String(n).padStart(2,'0')
      // Only emit date (no time)
      const s = `${this.y}-${pad(this.m)}-${pad(this.d)}`
    ...
    _parse(s) {
      ...
      if (!s || typeof s !== 'string') {
        ...
        return {
          ...
          h: 0,
          min: 0,
          str: `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}`
        }
      }
      // Expect 'YYYY-MM-DD' (date only, but tolerate time portion if present)
      try {
        const [datePart, timePart='00:00'] = s.replace('T',' ').split(/\s+/)
        const [Y,M,D] = datePart.split('-').map((x) => parseInt(x,10))
        const [h=0, m=0] = (timePart || '00:00').split(':').map((x) => parseInt(x,10))
        // Store time as 00:00 for date-only mode
        return { y:Y||now.getFullYear(), m:M||1, d:D||1, h: 0, min: 0 }
      } catch (e) {
...
.dtp-popover { position: absolute; margin-top: 8px; background: #fff; border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px; box-shadow: 0 10px 20px rgba(0,0,0,0.08); z-index: 50; }
.dtp-main { display: grid; grid-template-columns: 1fr; gap: 10px; width: 300px; }
...
.ym-vertical { font-weight: 600; cursor: pointer; }
