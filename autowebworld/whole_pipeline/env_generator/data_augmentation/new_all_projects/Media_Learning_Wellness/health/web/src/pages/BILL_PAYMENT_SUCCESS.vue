  ...
  <div class="min-h-screen bg-white flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 text-center">
       ...
       <div class="bg-gray-50 rounded-lg p-4 mt-8">
          <p class="text-xs text-gray-500 uppercase tracking-wide">Confirmation ID</p>
          <p class="text-2xl font-mono font-bold text-[#005DAA] mt-1">{{ store.payment_confirmation }}</p>
       </div>

       <div class="mt-8 space-y-4">
          <button
            id="payment-success-go-dashboard"
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      store.payment_confirmation = "PAYCONF123"
    })

    const handleGoDashboard = async () => {
      // ACT_PAYMENT_SUCCESS_GO_DASH
      store.setCurrentPageId('DASHBOARD')
      await router.push({ name: 'DASHBOARD' })
    }

    const handleGoHome = async () => {
      // ACT_PAYMENT_SUCCESS_GO_HOME
