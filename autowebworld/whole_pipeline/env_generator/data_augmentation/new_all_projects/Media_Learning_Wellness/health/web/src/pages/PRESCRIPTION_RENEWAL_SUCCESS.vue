<template>
  <div class="min-h-screen bg-white flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8 text-center">
       ...
       <h2 class="mt-6 text-3xl font-extrabold text-gray-900">Renewal Requested</h2>
       
       <p class="mt-2 text-sm text-gray-600">
         Your renewal request has been sent to your provider.
       </p>

       <div class="bg-gray-50 rounded-lg p-4 mt-8">
          <p class="text-xs text-gray-500 uppercase tracking-wide">Confirmation Number</p>
          <p class="text-2xl font-mono font-bold text-[#005DAA] mt-1">{{ store.renewal_confirmation }}</p>
       </div>

       <div class="mt-8 space-y-4">
          <button
            id="rx-success-go-dashboard"
...
export default {
  name: 'PRESCRIPTION_RENEWAL_SUCCESS',
  setup() {
    const router = useRouter()
    const store = useSignatureStore()

    onMounted(() => {
      // Set confirmation for display (Effect logic ACT_RX_SUCCESS_GO_HOME sets it, but we show it here)
      store.renewal_confirmation = "RXCONF123"
    })

    const handleGoDashboard = async () => {
      // ACT_RX_SUCCESS_GO_DASH
      store.setCurrentPageId('DASHBOARD')
      await router.push({ name: 'DASHBOARD' })
    }

    const handleGoHome = async () => {
      // ACT_RX_SUCCESS_GO_HOME
