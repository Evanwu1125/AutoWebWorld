<template>
  <div class="success-page min-h-screen bg-gray-50 flex items-center justify-center p-4">
    <div class="bg-white max-w-md w-full rounded-2xl shadow-xl overflow-hidden text-center p-8 animate-fade-in-up">
      ...
      <p class="text-gray-600 mb-6">Your order #{{ store.order_id }} has been placed successfully.</p>
      
      <div class="bg-gray-50 rounded-xl p-4 mb-8 text-left border border-gray-100">
        <h3 class="font-bold text-sm text-gray-500 uppercase mb-2">Delivery Details</h3>
        <p class="font-medium">Arriving by tomorrow, 8pm</p>
        <p class="text-sm text-gray-500 mt-1">We've sent a confirmation email to you.</p>
      </div>
...
export default {
  name: 'CHECKOUT_DELIVERY_SUCCESS',
  setup() {
    const router = useRouter()
    const store = useSignatureStore()

    const handleViewOrders = async () => {
      // FSM: ACT_DELIVERY_SUCCESS_VIEW_ORDERS
      store.currentPageId = 'ORDER_HISTORY'
      await router.push({ name: 'ORDER_HISTORY' })
    }

    const handleGoHome = async () => {
      // FSM: ACT_DELIVERY_SUCCESS_GO_HOME
