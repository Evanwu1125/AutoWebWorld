#!/usr/bin/env python3
"""Split FSM filter operations into atomic operations"""
import json
import copy
from typing import Dict, List, Any

def split_filter_gui_procedures(gui_procedures: List[Dict]) -> List[List[Dict]]:
    """Split filter into atomic operations: click+type_text(search) / click+drag(slider) / hover+click(dropdown) / click(checkbox)"""
    atomic_groups = []
    i = 0
    ...
    while i < len(gui_procedures):
        ...
        # Pattern 0: click + type_text (search operation)
        ...
        # Pattern 2: hover + click (dropdown)
        ...
        # Pattern 3: standalone click (checkbox)
        if op == 'click':
...
def create_atomic_filter_action(base_action: Dict, group_index: int, gui_group: List[Dict],
    ...
    if len(gui_group) == 2:
        ...
        # standalone click -> CHECKBOX
        suffix = 'CHECKBOX'
    else:
        suffix = f'FILTER_{group_index + 1}'

    # Append index for filters of the same type
...
def normalize_keyboard_keys(fsm: Dict[str, Any]) -> Dict[str, Any]:
    """
    Normalize keyboard keys in the FSM, replacing Ctrl with Control (Playwright compatible)

    Iterates over all pages -> actions -> gui_procedure, finds steps where op is key_press,
    and replaces "Ctrl+" with "Control+" in the key field
    """
    modified_count = 0
...
def process_fsm(fsm: Dict[str, Any]) -> Dict[str, Any]:
    """Process the FSM, splitting filter operations into atomic operations"""
    ...
    for page in new_fsm.get('pages', []):
        ...
        for action in page.get('actions', []):
            ...
            # Split filter operations
            if 'APPLY' in action_id and 'FILTER' in action_id:
                print(f"Splitting filter operation: {action_id}")
                gui_procedures = action.get('gui_procedure', [])
                
                if len(gui_procedures) > 2:  # Only split complex filters
                    atomic_groups = split_filter_gui_procedures(gui_procedures)
                    print(f"  Split into {len(atomic_groups)} atomic operations")
                    ...
                    # Simple filters are left unchanged
                    new_actions.append(action)
            else:
                # Other operations are left unchanged
                new_actions.append(action)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='Split filter operations in FSM into atomic operations')
    ...
    # Normalize keyboard keys (Ctrl -> Control)
    ...
    print(f"\n✅ Processing complete! New FSM saved to: {args.output}")
